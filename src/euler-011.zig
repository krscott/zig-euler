...
const Grid = struct {
    ...
    pub fn initFromString(allocator: Allocator, s: []const u8) !Self {
        ...
        while (i < s.len) : (i += 1) {
            switch (s[i]) {
                '0'...'9' => {
                    const val = try std.fmt.parseUnsigned(u8, s[i .. i + 2], 10);
                    try cells.append(val);
                    col += 1;

                    // Extra i increment since we're parsing 2 chars
                    i += 1;
                },
                ' ' => {},
                '\n' => {
    ...
    fn index(self: *const Self, row: usize, col: usize) ?usize {
        ...
        const i: usize = row * self.height + col;
    ...
    pub fn get(self: *const Self, row: usize, col: usize) ?u8 {
        const i = self.index(row, col) orelse return null;
        const out = self.cells.items[i];
        return out;
    }
};

fn getSegmentProduct(grid: Grid, length: usize, r: usize, c: usize, dr: isize, dc: isize) ?u64 {
    var prod: u64 = 1;
    var i: isize = 0;
    while (i < length) : (i += 1) {
        prod *= grid.get(r +% @bitCast(usize, i * dr), c +% @bitCast(usize, i * dc)) orelse return null;
    }
    return prod;
}

fn findGreatestProduct(grid: Grid, length: usize) u64 {
    var max_prod: u64 = 0;
    ...
    while (r < grid.height) : (r += 1) {
        ...
        while (c < grid.width) : (c += 1) {
            max_prod = max(max_prod, getSegmentProduct(grid, length, r, c, 1, 0) orelse 0);
            max_prod = max(max_prod, getSegmentProduct(grid, length, r, c, 1, 1) orelse 0);
            max_prod = max(max_prod, getSegmentProduct(grid, length, r, c, 0, 1) orelse 0);
            max_prod = max(max_prod, getSegmentProduct(grid, length, r, c, 1, -1) orelse 0);
        }
    }
...
fn findGreatestProductFromString(allocator: Allocator, input: []const u8, length: usize) !u64 {
    ...
    return findGreatestProduct(grid, length);
}

test "simple problem 1" {
    const smolgrid =
        \\14 01 01 01 01 01
        \\01 26 01 01 01 01
        \\01 01 63 01 01 01
